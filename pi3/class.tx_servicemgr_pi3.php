...
class tx_servicemgr_pi3 extends tx_servicemgr {
	...
	function showUpload($eventId) {
		...
		if($duty[$this->generalConf['PreacherTeamUID']] !== null && count($allPreachers) !== 0) {
			...
			if (count($actPreachers) > 1) {
				$outputPreachers = '<select size="3" name="'.$this->prefixId.'[upload][preachers][]" multiple="multiple">
				';
			} else {
				$outputPreachers = '<select size="1" name="'.$this->prefixId.'[upload][preachers][]">
				';
			}
